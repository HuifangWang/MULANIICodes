function mln_Cos_BtN_mulan(dirname,prenom)
   % Calculate the Cos similarity of mulan according to the varying number
   % of bootstraping
   %example:dirname='/Volumes/HWMac1/MULAN2/SPM/nmmN30'
   %prenom='Sta_nmmN30L5CS80S1N6100th125060.mat'
   %
   if dirname(1)=='/'
       dirname=dirname;
   else
   ...
   nbvector=[10,20,30,50,100,150,200,300];
   
   %nbvector=[10,20];
   tosavefile=[dirname,'/ToutResults/BtN_COS_',prenom];
   if ~exist(tosavefile,'file')
    ...
    if exist(neededfilemln,'file')
        ...
        MatRef=xinfo.MatRef;
        Nnbvector=length(nbvector);
        iMULAN.AUC=NaN(NiG1,NiG3,NiG4,Nnbvector);
        iMULAN.MSim=NaN(NiG1,NiG3,NiG4,Nncof,Nnbvector);
        iMULAN.mlnMat=NaN(Nnode,Nnode,NiG1,NiG3,NiG4,Nncof,Nnbvector);
        %iMULAN.MSim=iMULAN.AUC;
        for iG1=1:NiG1
            for iG3=1:NiG3
                for iG4=1:NiG4
                    iMatiGMax=squeeze(iMULAN.Mat(:,:,iG1,iG3,iG4,:));
                    for inbv=1:Nnbvector
                        iMatiG=iMatiGMax(:,:,1:nbvector(inbv));
                        miMat=mean(iMatiG,3);
                        [~,~,~,~,~,~,~,~,auc,~,~] =mln_calc_FalseRate(miMat,MatRef,mln_issymetricM(Methods),1);
                        iMULAN.AUC(iG1,iG3,iG4,inbv)=auc;
                    % iMULAN.chis(iG1,iG3,iG4,ith1,ith2,imlnth)=auc;
                        for i_incof=1:Nncof
                             ncof=incof(i_incof);
                             [mlnMat,sim]=plot_mln_Sta_finalNetwork_Mat(iMatiG,MatRef,Theta,ncof);
                             iMULAN.MSim(iG1,iG3,iG4,i_incof,inbv)=sim;
                             iMULAN.mlnMat(:,:,iG1,iG3,iG4,i_incof,inbv)=mlnMat;
                        end
                    end
                end
        ...
        mMULAN.AUC=NaN(Nnbvector);
        mMULAN.MSim=NaN(Nncof,Nnbvector);
        mMULAN.mlnMat=NaN(Nnode,Nnode,Nncof,Nnbvector);
        
        %% for average
           for inbv=1:Nnbvector
                    MULAN3d=mMULAN.Mat(:,:,1:nbvector(inbv));
                    iMat=squeeze(mean(MULAN3d,3));
                    ...
                    [mlnMat,sim]=plot_mln_Sta_finalNetwork_Mat(iMat,MatRef,Theta,ncof);
                               mMULAN.MSim(i_incof,inbv)=sim;
                               mMULAN.mlnMat(:,:,i_incof,inbv)=mlnMat;
                               end
                     xinfo.nbvector=nbvector;          
