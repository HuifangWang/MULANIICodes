function outputname=mln_alg_output_wins(dir,prenom,threshold1,threshold2,kforMF,Fthreshold,nboot,nboot2,BMG,Nwins,add)
...
if ~exist(savestatisfile,'file')
    ...
    Nwin=size(Mati.(iMed{1}),3);
    
    if nargin<10
        Nwins=1:min(15,Nwin);
    else
       if Nwins(end)>Nwin
        Nwins=Nwins(1:find(Nwins>=Nwin,1));
      end 
    end
        %%
    xinfo.Nwins=Nwins;
    % read all used methods
    for iwins=1:length(Nwins)
        for img=1:NmethodG
            methodlog=BMG{img};
            MatA=load(dataprenom,methodlog{:});
            InputMM=mln_GenerateTDM_win(MatA,methodlog,nboot,nboot2,Nwins(iwins));
...
function Input=mln_GenerateTDM_win(MatA,methods_used,nboot,nboot2,maxwin)
