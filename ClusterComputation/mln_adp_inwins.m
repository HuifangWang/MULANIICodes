function outputname=mln_adp_inwins(dir,prenom,para,inwins,add,indtype,opt)
% this function is to output mulan for different numbers of windows
% tak fb is to fix the parameters across the different time segments

dir=[dir,'/ToutResults/'];

nparacal=length(para.name);
...
switch indtype
    case 'tak'
        ...
        Populations=opt.BestFPNch(ind,1:nparacal);
        para.theta=opt.BestFPNch(ind,nparacal+1:npara);
    ...
    Populations=tPopulations(:,1:npara-1);
    para.theta=tPopulations(:,npara);
    case 'hp'
        pE=opt.hp;
        var=opt.var;
    tPopulations=mln_normrnd(pE,var,para.nboot2); % here nboots is the number of groups of parameters
