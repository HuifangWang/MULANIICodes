function [y,v] = spm_int_Lx0(x0,P,M,U)
...
% Karl Friston
% $Id: spm_int_L.m 5692 2013-10-13 13:44:05Z karl $
...
D    = 1;
if nargout(f) >= 3
    [fx, dfdx,D] = spm_fx_erp(x,u,P,M);
    
elseif nargout(f) == 2
    [fx, dfdx]   = spm_fx_erp(x,u,P,M);
...
for i = 1:size(U.u,1)
    ...
    try
        ...
        %------------------------------------------------------------------
        for j = 1:N
            v = v + Q*spm_fx_erp(v,u,P,M);
        end
        
        % output - implement g(x)
        %------------------------------------------------------------------
        y(:,i) = spm_gx_erp(v,u,P,M);
        ...
        %------------------------------------------------------------------
        for j = 1:N
            x = spm_vec(x) + Q*spm_vec(spm_fx_erp(x,u,P,M));
            x = spm_unvec(x,M.x);
        end
