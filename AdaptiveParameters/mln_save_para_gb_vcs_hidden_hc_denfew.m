function mln_save_para_gb_vcs_hidden_hc_denfew()
...
basicdir='/Users/huifangwang/MULANa/humanCon/HC84S50T/hidden/';
filesubdir='ToutResults/';
Nbins=20;

allbgv=nchoosek(1:6,2);
%
...
denbgv=[nchoosek(1:3,2);nchoosek(4:6,2)];
fewbgv=mln_remove(allbgv,denbgv);
...
for indBG =1:length(BG)
    ...
    savefilename=[basicdir,'opt_para_FRN_hidden_',num2str(nc),addname,'.mat'];
...
function allbgv=mln_remove(allbgv,denbgv)
for idb=1:length(denbgv)
    for ibgv=1:length(allbgv)
    if isequal(allbgv(ibgv,:),denbgv(idb,:))
        allbgv(ibgv,:)=[];
        break
    end
    end
