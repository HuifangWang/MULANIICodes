...
% Jan 30, 2015 add the addition name

function mln_Result2file_add(dirname,dataprenom,GroupMethlog,add)
...
for igroup=1:length(GroupMethlog)
    Resultfile=[dirname,'/Results/',GroupMethlog{igroup},'_',add,'_',dataprenom];
    ...
    for imethods=1:length(fieldname)
        ...
        if length(size(iNetSaved.(Methlog).Mat))<4
...
filec=load(datafile);
if isfield(filec,'Connectivity')
    Connectivity=filec.Connectivity;
%Connectivity=mln_normalizeNet(Connectivity-diag(diag(Connectivity)));
NewStru.Connectivity=Connectivity;
end

filesaved=[dirname,'/ToutResults/Tout_',add,'_',dataprenom];
