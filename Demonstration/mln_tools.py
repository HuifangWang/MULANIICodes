...
def mln_generateColormap(levelmap,lcolorname='white',nleeds=64):
    listmap=[]
    fcolor=hex_to_rgb(cm.cnames[levelmap])
    lcolor=hex_to_rgb(cm.cnames[lcolorname])
    
    dicolor=(np.array(fcolor)-np.array(lcolor))/float(np.array(nleeds))
    for inode in np.arange(nleeds):
        icolor=dicolor*inode+lcolor
        listmap.append(icolor/255.)
    return listmap

def ns_channel_color_LFP(area_color,labelname):
    
    Nchan = len(labelname)
    Color_ch = np.empty([Nchan,4])
    analabelname = [None]*Nchan
    for iarea, iele,leeds, mycolor in area_color:
        #print mycolor
        for igleeds in leeds:
            if np.size(igleeds)>1:
                for ileedsName in np.arange(igleeds[0],igleeds[1]+igleeds[2],igleeds[2]):
                    ileedsCh= iele+str(ileedsName)
                    analabelname[labelname.index(ileedsCh)]=ileedsCh+'_'+iarea
                    Color_ch[labelname.index(ileedsCh),:]=mycolor
            else:
                ileedsCh= iele+str(igleeds[0])
                analabelname[labelname.index(ileedsCh)]=ileedsCh+'_'+iarea
                Color_ch[labelname.index(ileedsCh),:]=mycolor
    return Color_ch/255., analabelname

def mln_plt_Corr_Cos_Mat(fig,pltMethods,matdata,overlap,iwins,mode,flagdyStru,tstfwins=[], nrow=4, order=1, flagxy=0):
    # mode = {'Corr', 'Cos'}
    nBM=np.size(pltMethods)
    #cmap=plt.cm.seismic
    cmap=plt.cm.jet
    
    for index,iBMed in enumerate(pltMethods):
        ax = fig.add_subplot(nrow,(nBM+1)/nrow+1,index+1)
        BMwins=matdata[iBMed]
        BMwins=np.abs(BMwins)
        if mode == 'Corr':
            CorrM=mln_calCorrM(BMwins,order)
        elif mode == 'Cos':
            CorrM=mln_calCosM(BMwins,order)
        #CorrM=mt.mln_norm(CorrM,mode='diag')
        cax=plt.imshow(CorrM,cmap=cmap,interpolation='nearest')
        if flagxy==1:
            ax.xaxis.set_ticks(np.arange(len(CorrM)))
            ax.yaxis.set_ticks(np.arange(len(CorrM)))
            ax.xaxis.set_ticklabels(np.arange(len(CorrM))*iwins*overlap)
            ax.yaxis.set_ticklabels(np.arange(len(CorrM))*iwins*overlap)
        else:
            ax.xaxis.set_ticks([])
            ax.yaxis.set_ticks([])
        
        if flagdyStru==1:
            lline=tstfwins.flatten()
            lline=lline[:-1]-1.5
            for illine in lline:
        ...
        axbar=fig.add_axes([0.723,0.14,0.01,0.15])
...
def mln_read_fmlnMat(filename,nc):
    
    fmlnMat=np.zeros([nc,nc])
    mlnMat,para=mln_read_mlnMat(filename)
    methodlog=para['methodlog']
    theta=para['theta']
    fmlnMat=np.mean(mlnMat,axis=2)
    return fmlnMat
